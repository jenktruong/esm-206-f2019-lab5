...
- learn to pars dates with 'lubridate'
...
Use tidy::uncount() function to convert our lobster data from frequency format to case format
...
Now let's arse year and month using lubridate::month() and lubridate::year():
...
Now let's find counts of observed lobsers based on different hierarchical groupings:
...
If we want to create a summary table that contains statsitics OTHER than counts by groups, it's easier to use group_by() + n()
